...
namespace Tools.PreviewModule
{
    public struct EffectInfo
    {
        ...
        public void FillMenu(GenericMenu menu)
        {
            AddMenu(menu, $"物体数量\t{goCount}");
            AddMenu(menu, $"渲染物数量\t{rendererCount}");
            AddMenu(menu, $"发射器数量\t{particleSystemCount}");
            AddMenu(menu, $"粒子总数\t{particleCount}");
            AddMenu(menu, $"动作资源数量\t{animCount}");
            AddMenu(menu, $"曲线数量总和\t{curveCount}");
            AddMenu(menu, $"关键帧平均值\t{keyframeCount}");
            AddMenu(menu, $"贴图资源数量\t{textureCount}");
            AddMenu(menu, $"等价尺寸\t{equalTextureSize}");
            AddMenu(menu, $"网格资源数量\t{meshCount}");
            AddMenu(menu, $"顶点总和\t{vertexCount}");
            AddMenu(menu, $"材质资源数量\t{materialCount}");
        }

        void AddMenu(GenericMenu menu, string label)
        {
            var temp = new GUIContent(label);
            menu.AddItem(temp, false, null);
        }
        ...
        public void Collect()
        {
            ...
            this.goCount = effect.GetComponentsInChildren<Transform>(true).Length;
...
#pragma warning restore CS0618 // 类型或成员已过时
            ...
            this.rendererCount = effect.GetComponentsInChildren<Renderer>(true).Length;
